...
const WhyPortfolioSection: React.FC<WhyPortfolioSectionProps> = ({ isLoaded }) => {
  const cards = [
    {
      icon: Star,
        ...
        <div className={`transition-all duration-1000 ${isLoaded ? 'animate-fade-up' : 'opacity-0 translate-y-8'}`}>
          <div className="text-center mb-16 md:mb-20">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto leading-relaxed">
              In today's competitive landscape, a portfolio is more than just a nice-to-have—it's your competitive edge
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 max-w-6xl mx-auto">
            {cards.map((card, index) => (
              <div 
                key={index}
                className="text-center p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 group"
              >
